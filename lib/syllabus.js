...
const RETAIL_FIRST_YEAR = '/documents/syllabus/retail-management-first-year.pdf';
...
/**
 * One document rather than one per semester: the programme runs a single year
 * and BTEUP publishes both its semesters in the same paper.
 */
function retailYears(department) {
  return [
    {
      key: 'first',
      title: `First Year ${department.name} Syllabus`,
      note: 'Both semesters in one paper — retail operations and merchandising, then supply chain, customer relationship management and the industry project.',
      file: RETAIL_FIRST_YEAR,
    },
  ];
}
...
export function getSyllabus(slug) {
  ...
  return {
    ...department,
    // The paragraph describing the scheme changes with the programme — a one
    // year PG diploma has no first year common to anything, and nothing after
    // it to specialise into.
    ...
    years: isRetail ? retailYears(department) : diplomaYears(department, slug),
  };
}
