...
export function getSyllabus(slug) {
  // Gated on SECOND_YEAR, not on DEPARTMENTS — only the three diploma branches
  // have a BTEUP syllabus; the other departments listed there have none, and
  // must 404 rather than render a page of missing download links.
  const department = SECOND_YEAR[slug] ? DEPARTMENTS[slug] : null;
