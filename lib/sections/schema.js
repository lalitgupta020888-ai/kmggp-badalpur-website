...
export const SECTIONS = {
  ...
  'welcome-popup': {
    collection: 'welcome-popup',
    title: 'Welcome Popup',
    icon: 'bi-window-stack',
    group: 'Home Page',
    type: 'single',
    intro:
      'The window that greets a visitor when the site opens. Use it for an admission deadline, a notice, a poster or a film. Switch it off entirely with the tick box below — nothing else needs changing.',
    fields: [
      {
        name: 'enabled',
        label: 'Show the popup when the site opens',
        type: 'check',
        full: true,
      },
      {
        ...
        type: 'select',
        full: true,
        options: ['Notice', 'Image', 'Film', 'Gallery film'],
        hint: 'Notice = heading and text · Image = a poster · Film = an uploaded video · Gallery film = whichever album currently carries a film.',
      },
      { name: 'title', label: 'Heading', type: 'text', full: true, placeholder: 'Admissions 2026-27 are open' },
      {
        name: 'message',
        label: 'Message',
        type: 'textarea',
        full: true,
        hint: 'Shown for a Notice, and under the picture for an Image.',
      },
      { name: 'image', label: 'Poster image', type: 'media', kind: 'image', full: true },
      { name: 'video', label: 'Film', type: 'media', kind: 'video', full: true },
      { name: 'poster', label: 'Film poster', type: 'media', kind: 'image', full: true },
      iconField(),
      { name: 'buttonLabel', label: 'Button text', type: 'text' },
      { name: 'buttonHref', label: 'Button goes to', type: 'link' },
      iconField('buttonIcon', 'Button icon'),
      {
        name: 'frequency',
        label: 'How often a visitor sees it',
        type: 'select',
        full: true,
        options: ['Every visit', 'Once a day', 'Once only'],
        hint: '“Once a day” and “Once only” are remembered in the visitor’s own browser. Changing the heading starts everyone afresh, so a new announcement is always seen.',
      },
    ],
  },
  ...
  'academic-calendar': {
    ...
    group: 'Academics',
    type: 'single',
    intro: 'The semester schedule on /academic/calendar, and the calendar PDF offered alongside it.',
    fields: [
      { name: 'file', label: 'Academic calendar (PDF)', type: 'media', kind: 'document', full: true },
      {
        name: 'oddSemester',
        label: 'Odd semester',
        type: 'items',
        ...
        fields: [
          ...
          { name: 'date', label: 'When', type: 'text', placeholder: 'October 2026' },
          iconField(),
        ],
      },
      {
        ...
        fields: [
          { name: 'event', label: 'Event', type: 'text' },
          { name: 'date', label: 'When', type: 'text', placeholder: 'March 2027' },
          iconField(),
        ],
      },
    ],
  ...
  holidays: {
    ...
    intro: 'Gazetted and institutional holidays listed on /academic/holidays.',
    summary: { title: 'occasion', meta: ['date', 'day'] },
    fields: [
      { name: 'occasion', label: 'Occasion', type: 'text', full: true },
      { name: 'date', label: 'Date', type: 'text', placeholder: 'August 15, 2026' },
      {
        name: 'day',
        label: 'Day',
        type: 'select',
  ...
  'department-pages': {
    collection: 'department-pages',
    title: 'Department Page Text',
    icon: 'bi-file-text-fill',
    group: 'Departments',
    type: 'single',
    intro:
      'The wording at the top of each department sub-page. The lists themselves — faculty, laboratories, achievements — are edited per department under Departments; this is the shared prose that introduces them.',
    fields: [
      {
        name: 'homeIntroHeading',
        label: 'Department home — panel heading',
        type: 'text',
        full: true,
        hint: 'The department name is added after this, e.g. “Welcome to” becomes “Welcome to Information Technology”.',
      },
      { name: 'facultyIntro', label: 'Faculty page — opening line', type: 'textarea', full: true },
      { name: 'facultyNote', label: 'Faculty page — closing note', type: 'textarea', full: true },
      { name: 'labsIntro', label: 'Laboratories page — opening line', type: 'textarea', full: true },
      { name: 'achievementsIntro', label: 'Achievements page — opening line', type: 'textarea', full: true },
    ],
  },
  ...
  'fee-structure': {
    ...
    type: 'single',
    intro:
      'The fee table on /admission/fee, the government order behind it, the concessions available and the ways a fee can be paid.',
    fields: [
      {
        name: 'goFile',
        label: 'Government order (PDF)',
        type: 'media',
        kind: 'document',
        full: true,
        hint: 'The sanctioned fee notification visitors download from this page.',
      },
      {
        name: 'fees',
        label: 'Fee heads',
        ...
        fields: [
          { name: 'head', label: 'Fee head', type: 'text' },
          { name: 'amount', label: 'Amount', type: 'text', placeholder: '₹ 10,370' },
          { name: 'period', label: 'Charged', type: 'select', options: ['Per Annum', 'Per Semester', 'One Time'] },
          iconField(),
        ],
      },
      {
        ...
        fields: [{ name: 'text', label: 'Concession', type: 'textarea', full: true }],
      },
      {
        name: 'paymentModes',
        label: 'Ways to pay',
        ...
        fields: [
          { name: 'label', label: 'Payment mode', type: 'text' },
          iconField(),
        ],
      },
    ],
  ...
  'aicte-letters': {
    ...
    addLabel: 'Add letter',
    intro:
      'The Extension of Approval letters listed on /about/aicte-approvals. Upload the PDF against a session and the row becomes a real download; without one it is shown as text, so nothing links nowhere.',
    summary: { title: 'session', meta: ['type', 'status'] },
    fields: [
      { name: 'session', label: 'Session', type: 'text', placeholder: '2026-27' },
      { name: 'type', label: 'Letter type', type: 'text', placeholder: 'Extension of Approval (EOA)' },
      { name: 'status', label: 'Status', type: 'select', options: ['Approved', 'Applied', 'Pending'] },
      { name: 'file', label: 'Letter (PDF)', type: 'media', kind: 'document', full: true },
    ],
  },
  ...
  booklet: {
    ...
    type: 'single',
    intro:
      'The Information Booklet page. A download with no file attached is shown as plain text rather than a link, so you can list a document before it is ready.',
    fields: [
      { name: 'title', label: 'Booklet name', type: 'text', full: true, placeholder: 'Information Booklet 2026-27' },
      { name: 'file', label: 'Booklet (PDF)', type: 'media', kind: 'document', full: true },
      {
        name: 'contents',
        label: 'What the booklet covers',
        type: 'items',
        full: true,
        addLabel: 'Add line',
        fields: [{ name: 'text', label: 'Covers', type: 'text', full: true }],
      },
      {
        name: 'downloads',
        ...
        fields: [
          { name: 'label', label: 'Document name', type: 'text', full: true },
          iconField(),
          { name: 'file', label: 'PDF', type: 'media', kind: 'document', full: true },
        ],
      },
    ],
  },
