...
export const DEPARTMENTS = {
  ...
  'retail-management': {
    ...
    icon: 'bi-shop',
    // A one year post graduate diploma, not a three year branch. `seats` is
    // left unset until the sanctioned intake is confirmed — the figure tile is
  ...
  'applied-sciences': {
    ...
    icon: 'bi-rulers',
    // No seats and no duration: it admits no cohort of its own, it teaches the
    // common first year for every branch. Both tiles are dropped accordingly.
  ...
  /**
   * Not a teaching department — it carries no programme copy, focus areas or
   * laboratories, and the `staff` array below is the whole of its section: a
   * photograph and the person's details, nothing else. The presence of `staff`
   * is what switches the department pages into that listing-only mode.
   *
   * `photo: null` renders the gold initial placeholder, the same convention
   * PrincipalCard uses. Drop a portrait into `public/images/staff/` and set
   * `photo` to its path to replace it.
   */
  'non-technical-staff': {
    slug: 'non-technical-staff',
    name: 'Non-Technical Staff',
    short: 'Non-Technical Staff',
    icon: 'bi-people-fill',
    eyebrow: 'Institute Staff',
    tagline: 'The administrative and support team of the institute',
    intro: '',
    focus: [],
    labs: [],
    staff: [
      {
        ...
        detail: 'Establishment and administrative correspondence',
        photo: null,
      },
      {
        name: 'Staff Member Two',
        designation: 'Accountant',
        detail: 'Accounts, fee collection and scholarship disbursement',
        photo: null,
      },
      {
        name: 'Staff Member Three',
        designation: 'Store Keeper',
        detail: 'Stores, stock register and procurement records',
        photo: null,
      },
      {
        name: 'Staff Member Four',
        designation: 'Library Assistant',
        detail: 'Library issue, return and catalogue maintenance',
        photo: null,
      },
      {
        name: 'Staff Member Five',
        designation: 'Lab Attendant',
        detail: 'Laboratory upkeep and equipment handling',
        photo: null,
      },
      {
        name: 'Staff Member Six',
        designation: 'Peon',
        detail: 'Office support and campus assistance',
        photo: null,
      },
    ],
  },
...
/** A staff listing has no faculty, laboratories or achievements to link to. */
const STAFF_NAV = [{ path: '', label: 'Staff Directory', icon: 'bi-people-fill' }];

export function getDeptNav(department) {
  return department.staff ? STAFF_NAV : DEPT_NAV;
}

/** True for departments whose only page is the staff listing. */
export function isStaffOnly(slug) {
  return Boolean(DEPARTMENTS[slug]?.staff);
}
